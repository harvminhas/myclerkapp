 ...
 * @author Rav
 * To add new request
 * 1. Add method to display to get link
 * 2. In bind method add click on link handler
 *
 */
public class CreateRequestPresenter extends AbstractPresenter {
	...
	public interface Display{
		...
		HasClickHandlers getMovingLink();
	    Widget asWidget();
	  }
	public CreateRequestPresenter(MyClerkServiceAsync rpcService, HandlerManager eventBus, Display view){
	...
	private void bind(){	
		super.commonBind();
		clickRegistration = display.getResiPlumbingLink().addClickHandler(new ClickHandler() { 
			
		      public void onClick(ClickEvent event) {
		    	  System.out.println("Plumbing - Get Quote button clicked.");
		    	  firePlumbingRequestEvent();
		    	
		      }
		    });
		
		movingClickRegistration = display.getMovingLink().addClickHandler(new ClickHandler() { 
			
		      public void onClick(ClickEvent event) {
		    	  System.out.println("Moving - Get Quote button clicked.");
